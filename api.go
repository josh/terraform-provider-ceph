...
type CephAPICephFSListEntry struct {
	ID     int                 `json:"id"`
	MdsMap CephAPICephFSMdsMap `json:"mdsmap"`
}

type CephAPICephFSMdsMap struct {
	FSName       string `json:"fs_name"`
	MetadataPool int    `json:"metadata_pool"`
	DataPools    []int  `json:"data_pools"`
}

type CephAPICephFSCreateRequest struct {
	Name        string                 `json:"name"`
...
func (c *CephAPIClient) CephFSList(ctx context.Context) ([]CephAPICephFS, error) {
	...
	var listEntries []CephAPICephFSListEntry
	err = json.Unmarshal(body, &listEntries)
	if err != nil {
		return nil, fmt.Errorf("unable to decode JSON response: %w", err)
	}

	filesystems := make([]CephAPICephFS, len(listEntries))
	for i, entry := range listEntries {
		filesystems[i] = CephAPICephFS{
			ID:             entry.ID,
...
func (c *CephAPIClient) CephFSGetByName(ctx context.Context, name string) (*CephAPICephFS, error) {
...
func (c *CephAPIClient) CephFSCreate(ctx context.Context, req CephAPICephFSCreateRequest) error {
	jsonPayload, err := json.Marshal(req)
	if err != nil {
		return fmt.Errorf("unable to encode request payload: %w", err)
	}
...
func (c *CephAPIClient) CephFSDelete(ctx context.Context, name string) error {
...
type CephAPISubvolumeListEntry struct {
	Name string                `json:"name"`
	Info *CephAPISubvolumeInfo `json:"info,omitempty"`
}

type CephAPISubvolumeInfo struct {
...
type CephAPISubvolumeCreateRequest struct {
...
func (info *CephAPISubvolumeInfo) BytesQuotaInt64() (int64, bool) {
...
func (c *CephAPIClient) CephFSSubvolumeList(ctx context.Context, volName string, groupName string) ([]CephAPISubvolumeListEntry, error) {
	...
	var entries []CephAPISubvolumeListEntry
...
func (c *CephAPIClient) CephFSSubvolumeInfo(ctx context.Context, volName string, subvolName string, groupName string) (*CephAPISubvolumeInfo, error) {
...
func (c *CephAPIClient) CephFSSubvolumeCreate(ctx context.Context, req CephAPISubvolumeCreateRequest) error {
	jsonPayload, err := json.Marshal(req)
	if err != nil {
		return fmt.Errorf("unable to encode request payload: %w", err)
	}
...
func (c *CephAPIClient) CephFSSubvolumeDelete(ctx context.Context, volName string, subvolName string, groupName string) error {
	endpoint := c.endpoint.JoinPath("/api/cephfs/subvolume", volName)
	query := url.Values{}
	query.Add("subvol_name", subvolName)
	if groupName != "" {
